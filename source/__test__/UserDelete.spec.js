...
describe('User Delete', () => {
  it('return forbidden when request sent sent unauthorized', async () => {
  ...
  it.each`
    language | message
    ${'vi'}  | ${vi.unauthorized_user_delete}
  ...
  it('returns forbidden when delete request is sent with correct credentials but for different user', async () => {
    await addUser();
    const userToBeDelete = await addUser({ ...activeUser, username: 'user2', mail: 'user2@mail.com' });
  ...
  it('deletes user in database when request sent from authorized user', async () => {
    const savedUser = await addUser();
    const token = await auth({ auth: { email: savedUser.email, password: 'P4ssword' } });
